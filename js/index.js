...
class Player {
    ...
    draw() {
        ctx.drawImage(this.image, this.x, this.y, this.width, this.height); // tekent de player op het canvas
    }
}
...
class ChickenNugget {
    ...
    draw() {
        ctx.drawImage(this.image, this.x, this.y, this.width, this.height); // tekent de kip nugget op het canvas
    }
}

// Spelers en objecten
let player1 = new Player(300, 103.5, player1Img); // Speler 1 wordt aangemaakt
let player2 = new Player(300, 455, player2Img); // Speler 2 wordt aangemaakt
let nuggets = []; // Array om kipnuggets in op te slaan
let score = 0;
let nuggetCounter = 0;
let gameRunning = true;

// Toetsenbord input
let keys = {}; // Dit houd bij welke toets word ingedrukt
...
function gameLoop() {
    ...
    if (keys[" "] && nuggets.length < 1) {
        nuggets.push(new ChickenNugget(player1.x + 25, 110, nuggetImg)); // Nieuwe nugget wordt gemaakt
        nuggetCounter++;
        knSound.play();
    }
